...
- `penhan update` updates penhan in place to the latest GitHub release. It verifies the archive against the release's `checksums.txt` and runs the new binary once before atomically replacing the old one. `--check` only reports. It never installs a release older than the running build, even with `--force` (which only moves a development build onto a release). It uses github.com release URLs rather than the REST API, which caps unauthenticated clients at 60 requests per hour per IP. Versions up to 0.5.1 don't have this command; install the next release manually once.
...
- `penhan add` no longer treats `/dev/null` as interactive stdin; in non-interactive runs (including redirected stdin and pipes) it now correctly fails fast with the missing-flags error instead of trying to open TTY prompts.
...
- `penhan add` now cleans up a newly created safe directory when setup fails partway through (for example, GitHub GPG key fetch failures), so rerunning `add` does not fail with "already initialized". Pre-existing directories are preserved.
